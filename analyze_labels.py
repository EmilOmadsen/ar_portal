...
print("Fetching top 300 songs from Chartex API...")
...
for label, count in label_counts.most_common():
    # Highlight major labels
    label_lower = label.lower()
    is_major = False
    major_type = ""
    
    if any(x in label_lower for x in ['universal', 'republic', 'def jam', 'interscope', 'island', 'capitol', 'motown']):
        is_major = True
        major_type = "[UNIVERSAL]"
    elif any(x in label_lower for x in ['sony', 'columbia', 'rca', 'epic', 'arista']):
        is_major = True
        major_type = "[SONY]"
    elif any(x in label_lower for x in ['warner', 'atlantic', 'elektra', 'parlophone', 'asylum']):
        is_major = True
        major_type = "[WARNER]"
    
    if is_major:
        print(f'{count:4d}x  {major_type:15s} {label}')
    else:
